...
if (isset($_SESSION["customer_email"])) {
 ...
 $insert_order = "insert into orders (customer_id,invoice_no,shipping_type,shipping_cost,
 payment_method,order_date,order_total,order_note,order_status) values ('$customer_id','$invoice_no',
 '$shipping_type_name','$shipping_cost','$payment_method','$order_date','$amount','$order_note','$status')";
 $run_order = mysqli_query($con,$insert_order);
 $insert_order_id = mysqli_insert_id($con);
 if ($run_order) {
   if ($is_shipping_address_same == "yes") {
     $insert_order_addresses = "insert into orders_addresses (order_id,billing_first_name,
     billing_last_name,billing_country,billing_address_1,billing_address_2,billing_state,
     billing_city,billing_postcode,is_shipping_address_same) values ('$insert_order_id',
     '$billing_first_name','$billing_last_name','$billing_country','$billing_address_1',
     '$billing_address_2','$billing_state','$billing_city','$billing_postcode','yes')";

   }elseif ($is_shipping_address_same == "no") {
     $insert_order_addresses = "insert into orders_addresses (order_id,billing_first_name,
     billing_last_name,billing_country,billing_address_1,billing_address_2,billing_state,
     billing_city,billing_postcode,is_shipping_address_same,shipping_first_name,shipping_last_name,
     shipping_country,shipping_address_1,shipping_address_2,shipping_state,
     shipping_city,shipping_postcode) values ('$insert_order_id',
     '$billing_first_name','$billing_last_name','$billing_country','$billing_address_1',
     '$billing_address_2','$billing_state','$billing_city','$billing_postcode','no',
     '$shipping_first_name','$shipping_last_name','$shipping_country','$shipping_address_1',
     '$shipping_address_2','$shipping_state','$shipping_city','$shipping_postcode')";
   }
   ...
   if ($run_insert_order_addresses) {
     ...
     while ($row_cart = mysqli_fetch_array($run_cart)) {
      ...
      $insert_order_item = "insert into orders_items (order_id, product_id,price,qty) values
      ('$insert_order_id','$product_id','$product_price','$product_qty')";
    ...
    if ($run_cart_delete) {
      echo "<script>
        alert('Porosia juaj u krye me sukses. Faleminderit!');
